...
class Testing(unittest.TestCase):
    ...
    def test_hydrogen_rate_coefficients_have_expected_units_and_values(self):
        ...
        alpha_b_expected = 2.753e-14 * lam**1.5 * (1.0 + (lam / 2.740) ** 0.407) ** -2.242
        beta_expected = (
            1.17e-10 * temp_value**0.5 * np.exp(-157809.1 / temp_value) / (1.0 + temp5**0.5)
        )
        gamma_ion_expected = (
            2.54e-21 * temp_value**0.5 * np.exp(-157809.1 / temp_value) / (1.0 + temp5**0.5)
        )
        gamma_line_expected = 7.5e-19 * np.exp(-118348.0 / temp_value) / (1.0 + temp5**0.5)
        gamma_b_expected = (
            3.435e-30 * temp_value * lam**1.970 * (1.0 + (lam / 2.250) ** 0.376) ** -3.720
        )
        gamma_ff_expected = (
            1.42e-27
            * temp_value**0.5
