...
class HydrogenHeliumNetworkTests(unittest.TestCase):
    def _state(self):
        return {
            "rho_cgs_g_cm3": np.array([PROTON_MASS_CGS, 2.0 * PROTON_MASS_CGS]),
            "hydrogen_mass_fraction": 0.7,
            "helium_mass_fraction": 0.28,
            "temperature_cgs_K": np.array([1.0e4, 2.0e4]),
    ...
    def test_multigroup_rates_and_heating_are_finite(self):
        state = self._state()
        ngamma_cgs_cm3 = np.array([[1.0e-3, 2.0e-3], [3.0e-3, 4.0e-3]])

        rates = _rates(state, ngamma_cgs_cm3)
    ...
    def test_implicit_update_keeps_hydrogen_and_helium_fractions_bounded(self):
        ...
        ionization_fraction_implicit_update(state, ngamma_cgs_cm3, 1.0e10)
    ...
    def test_supercomoving_source_state_is_physical(self):
        ...
        np.testing.assert_allclose(state["temperature_cgs_K"], [temperature])
        np.testing.assert_allclose(state["specific_energy_cgs_erg_g"], [specific_internal])
        np.testing.assert_allclose(state["velocity_supercomoving_cgs_cm_s"], [velocity_super])
