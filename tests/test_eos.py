...
class Mesh:
    def __init__(self):
        self.geometry_state = MeshGeometryState.from_arrays(
            ...
            width_proper_code=np.ones(8),
            area_proper_code=np.ones(8),
            volume_proper_code=np.ones(8),
        )
...
class Testing(unittest.TestCase):
    ...
    def test_isothermal_energy_excludes_pressure_term(self):
        ...
        np.testing.assert_array_equal(
            eos.thermal_energy_density(pressure),
            np.zeros(2),
        )
    ...
    def test_isothermal_set_primitive_recovers_pressure_from_temperature(self):
        ...
        np.testing.assert_allclose(
            np.asarray(fluid.pre_proper_code),
            np.asarray(expected_pressure),
        )
