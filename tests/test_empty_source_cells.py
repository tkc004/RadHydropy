...
def test_explicit_thermal_source_skips_empty_cells():
    state = {
        ...
        "rho_cgs_g_cm3": np.array([2.0, 0.0]),
        "specific_total_energy_cgs_erg_g": np.array([10.0, 20.0]),
        "specific_kinetic_energy_cgs_erg_g": np.zeros(2),
        "gamma": 5.0 / 3.0,
        "mu": np.ones(2),
    }
    ...
    np.testing.assert_allclose(state["specific_total_energy_cgs_erg_g"], [11.0, 20.0])


def test_compton_source_skips_empty_cells():
    state = {
        "active": np.array([True, False]),
        "rho_cgs_g_cm3": np.array([1.0, 0.0]),
        "temperature_cgs_K": np.array([1.0, 42.0]),
