...
def write_initial_condition(config, filename):
    ...
    build_initial_condition(config).write(filename, validate=True)
...
def active_radarray(radarray, active_cells, ghost_cells, *, boundary=False):
    """Return active cells from either active-only or ghost-inclusive data."""
    expected = active_cells + 1 if boundary else active_cells
...
def save_profile_plot(snapshots, output, exponent):
    ...
    for index, (time_proper_yr, snapshot) in enumerate(snapshots):
        ...
        velocity_axis.plot(
            radius_proper_pc,
            velocity_proper_cgs_cm_s / 1.0e5,
            linestyle=style,
            color=color,
            label=label,
        )
    ...
    for filename in output_files(output_directory, output_config['filename_prefix']):
        ...
        time_proper_yr = (
            ...
            * float(snapshot.par.units.CodeUnits.time_unit.to_value(unyt.s))
            / (1.0 * unyt.yr).to_value(unyt.s)
        )
        times_yr.append(time_proper_yr)
        radii_cm.append(front_radius_cgs_cm(snapshot, config))
