...
def build_initial_condition(config):
    ...
    volume=4*np.pi/3*np.diff(boundary**3); cut=(center < quantity_to_value(ic["explosion_radius"],units.length_unit)) & (center >= start); energy=quantity_to_value(ic["explosion_energy"],units.energy_unit); pressure=(par["hydrodynamics"]["gamma"]-1)*energy/np.sum(volume[cut]); temp[cut]=np.asarray(Rsim(config["par"]).fluid.eos.temperature(rho[cut],np.full(np.count_nonzero(cut),pressure),mu[cut]),dtype=float)
    return make_initial_condition(config, boundary_proper_code=boundary, rho_proper_code=rho, vel_proper_code=np.zeros(n), temp_proper_code=temp, mu_dimensionless=mu, area_proper_code=4*np.pi*boundary[:-1]**2)
