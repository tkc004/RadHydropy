...
def build_initial_condition(config):
    ...
    cut = (coordinate_proper_unyt < ic["radius_explosion_proper"]) & (
        coordinate_proper_unyt >= boundary_proper_unyt[0]
    )
...
def plot_snapshot(filename, config, **kwargs):
    ...
    time_proper_cgs_s = (
        float(np.asarray(sim.fluid.time_proper_code).reshape(-1)[0]) * time_cgs_per_code
    )
    analytic_valid = None
    if time_proper_cgs_s > 0.0:
        ic = config["initial_condition"]
        (
            analytic_radius_cgs_cm,
            analytic_rho_cgs_g_cm3,
            analytic_velocity_cgs_cm_s,
            analytic_pressure_cgs_erg_cm3,
            _,
        ) = get_blastwave_solution(
