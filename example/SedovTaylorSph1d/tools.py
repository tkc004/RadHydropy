...
def build_initial_condition(config):
    ...
    boundary_proper_code=np.linspace(start,start+size,n+1); coordinate_proper_code=.5*(boundary_proper_code[:-1]+boundary_proper_code[1:]); rho_proper_code=np.full(n,quantity_to_value(ic["initial_density"],units.density_unit)); mu_dimensionless=np.full(n,ic["mean_molecular_weight"]); temp_proper_code=np.zeros(n)
