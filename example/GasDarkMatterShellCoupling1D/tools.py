...
def make_dark_matter(config):
    ...
    angular_momentum = np.full(
        count,
        float(initial_condition["dark_matter_angular_momentum_dimensionless"]),
    )
    mass_code = quantity_to_value(
        initial_condition["dark_matter_mass"],
        code_units.mass_unit,
    )
    softening_proper_code = quantity_to_value(
        initial_condition["dark_matter_softening"],
        code_units.length_unit,
    )
