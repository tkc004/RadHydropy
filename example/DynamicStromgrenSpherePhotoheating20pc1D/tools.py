...
def save_plot(mesh, fluid, config, figure_filename):
    ...
    density_reference = load_reference_profile(
        example_config.get("density_reference_filename", None),
        radius_unit,
        log_value=True,
    )
    velocity_reference = load_reference_profile(
        example_config.get("velocity_reference_filename", None),
        radius_unit,
        log_value=False,
    )
    pressure_reference = load_reference_profile(
        example_config.get("pressure_reference_filename", None),
        radius_unit,
        log_value=True,
    )
    neutral_fraction_reference = load_reference_profile(
        example_config.get("neutral_fraction_reference_filename", None),
        radius_unit,
        log_value=True,
    )
