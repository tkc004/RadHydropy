...
def build_initial_condition(config):
    ...
    temp_proper_code = np.zeros(n); volume_proper_code = np.full(n, quantity_to_value(par["mesh"]["area"], units.area_unit) * np.diff(boundary_proper_code)); cut = 1
    ...
    return make_initial_condition(config, boundary_proper_code=boundary_proper_code, rho_proper_code=rho_proper_code, vel_proper_code=np.zeros(n), temp_proper_code=temp_proper_code, mu_dimensionless=mu_dimensionless, area_proper_code=np.full(n, quantity_to_value(par["mesh"]["area"], units.area_unit)))
...
def ReadandPlot(filename, config, **kwargs):
    ...
    if time_proper_unyt > 0.0 * unyt.s:
        ...
        area_proper_unyt = par["mesh"]["area"]
