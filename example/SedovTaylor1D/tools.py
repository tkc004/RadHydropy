...
def build_initial_condition(config):
    ...
    temp_proper_unyt[cut] = (
        pressure_proper_unyt * mu_dimensionless[cut] * unyt.mp / (rho_proper_unyt[cut] * unyt.kb)
    ).to(unyt.K)
...
def plot_snapshot(filename, config, **kwargs):
    ...
    x = 0.5 * (boundary_active[:-1] + boundary_active[1:])
    ...
    plt.subplot(1, 3, 1)
    plt.plot(x[left_half], np.asarray(pre_proper_code)[left_half], **kwargs)
    plt.subplot(1, 3, 2)
    plt.plot(x[left_half], vel_proper_code[left_half], **kwargs)
    plt.subplot(1, 3, 3)
    plt.plot(x[left_half], rho_proper_code[left_half], **kwargs)
    ...
    if time_proper_unyt > 0.0 * unyt.s:
        ...
        (
            analytic_radius_unyt,
            analytic_density_unyt,
            analytic_velocity_unyt,
            analytic_pressure_unyt,
            shock_radius_unyt,
        ) = sa.get_blastwave_solution(
            explosion_energy_unyt,
            density_proper_unyt * area_proper_unyt,
            1,
            par["hydrodynamics"]["gamma"],
            0.0,
            time_proper_unyt,
        )
        ...
        plt.subplot(1, 3, 1)
        plt.plot(
            analytic_radius_unyt[analytic_left_half].in_cgs(),
            analytic_pressure_unyt[analytic_left_half].in_cgs(),
            color=color,
            linestyle="--",
        )
        plt.subplot(1, 3, 2)
        plt.plot(
            analytic_radius_unyt[analytic_left_half].in_cgs(),
            analytic_velocity_unyt[analytic_left_half].in_cgs(),
            color=color,
            linestyle="--",
        )
        plt.subplot(1, 3, 3)
        plt.plot(
            analytic_radius_unyt[analytic_left_half].in_cgs(),
            (analytic_density_unyt[analytic_left_half] / area_proper_unyt).in_cgs(),
            color=color,
            linestyle="--",
        )
