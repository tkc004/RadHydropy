...
def main(output_root=None):
    ...
    initial_config = {
        ...
        "_vel_supercomoving_code": -(scale_factor**2)
        * hubble
        * overdensity
        * x_comoving_code
        / 3.0,
        "_temp_supercomoving_code": np.full(
            count,
            float(initial_condition["temperature_proper"].to_value(units.temperature_unit))
            * scale_factor**2,
        ),
    ...
    rotational_before[valid] = (
        0.5 * angular_before[valid] ** 2 / (mass_before[valid] * radius_comoving_code[valid] ** 2)
    )
    ...
    rotational_after[valid] = (
        0.5 * angular_after[valid] ** 2 / (mass_after[valid] * radius_comoving_code[valid] ** 2)
    )
