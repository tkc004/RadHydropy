...
def build_initial_condition(config):
    ...
    velocity_proper_unyt = np.where(
        ...
        < 0.5 * (initial["radius_inner_proper"] + initial["radius_outer_proper"]),
        initial["vel_outflow_proper"],
        initial["vel_inflow_proper"],
    )
...
def load_output_state(filename, config):
    ...
    return {
        ...
        "boundary_proper_cgs_cm": snapshot.mesh.boundary_radarray.to(unyt.cm).value[
            first : first + count + 1
        ],
        "rho_proper_cgs_g_cm3": snapshot.fluid.rho_radarray.to(unyt.g / unyt.cm**3).value[physical],
        "vel_peculiar_proper_cgs_cm_s": snapshot.fluid.vel_radarray.to(unyt.cm / unyt.s).value[
            physical
        ],
