...
"""Helpers for spherical ballistic infall."""  # noqa: CPY001
...
from radhydropy.units import code_unit_scales, quantity_to_value, time_seconds
...
def spherical_cell_centers(boundary_proper_code):
    ...
    x_proper_code[mask] = (
        0.75
        * (boundary_proper_code[1:][mask] ** 4 - boundary_proper_code[:-1][mask] ** 4)
        / denom[mask]
    )
...
def point_mass_acceleration(point_mass, softening=0.0, code_unit_system=None):
    ...
    point_mass_g = (
        ...
        else float(point_mass) * (scales["mass_g"] if scales else 1)
    )
    soft = (
        softening.to_value(unyt.cm)
        if hasattr(softening, "to_value")
        else float(softening) * (scales["length_cgs_cm"] if scales else 1)
    )

    def acceleration(x_proper_code):
        radius_proper_cgs_cm = (
            x_proper_code.to_value(code_unit_system.length_unit)
            if hasattr(x_proper_code, "to_value") and code_unit_system is not None
            else np.asarray(x_proper_code, dtype=float)
        )
        if scales is not None:
            radius_proper_cgs_cm = radius_proper_cgs_cm * scales["length_cgs_cm"]
        radius_proper_cgs_cm = np.maximum(radius_proper_cgs_cm, soft)
        return (
            -GRAVITATIONAL_CONSTANT_CGS * point_mass_g / radius_proper_cgs_cm**2
        ) * ACCELERATION_UNIT
...
def ballistic_velocity_profile(
    ...
    t = (
        ...
        if code_unit_system is not None
        else float(time_proper_code) * unyt.s
    )
...
def build_initial_condition(config):
    ...
    writer.fluid.mu = np.full(n, float(ic["mean_molecular_weight"]))
    return writer
...
def plot_snapshot(filename, config, **kwargs):
    ...
    boundary_proper_code = np.asarray(
        sim.mesh.boundary_radarray.to_value(units.length_unit),
        dtype=float,
    )
    ...
    analytic_rho = ballistic_density_profile(
        ...
        quantity_to_value(ic["rho_reference_proper"], units.density_unit),
    )
    analytic_vel = (
        ballistic_velocity_profile(
            coordinate_proper_code,
            ic["point_mass"],
            time_proper_code,
            code_unit_system=units,
        ).to_value(unyt.cm / unyt.s)
        / units.velocity_in_cgs
    )
    plt.subplot(1, 2, 1)
