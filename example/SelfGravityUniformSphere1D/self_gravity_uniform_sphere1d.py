...
from example.SelfGravityUniformSphere1D.tools import et
...
def main(config_filename=DEFAULT_CONFIG):
    ...
    radius_proper_cgs_cm_unyt = (
        np.asarray(radius_proper_code, dtype=float) * sim.par.units.CodeUnits.length_unit
    )
    ...
    relative_error = np.abs(
        (numerical_cgs[comparison] - analytic_cgs[comparison]) / analytic_cgs[comparison],
    )
