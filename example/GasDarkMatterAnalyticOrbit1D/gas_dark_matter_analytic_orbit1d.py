...
from example.GasDarkMatterAnalyticOrbit1D import tools as et
...
def main(config_filename=DEFAULT_CONFIG):
    ...
    def rhs(time_proper_code, state):
        ...
        enclosed_mass_code = (
            ...
            + 4.0 * np.pi / 3.0 * gas_density_dimensionless * radius_code**3
        )
        acceleration = (
            -g_code * enclosed_mass_code / (radius_code + softening_length_dimensionless) ** 2
            + specific_angular_momentum_dimensionless**2
            / (radius_safe_code + softening_length_dimensionless) ** 3
        )
    ...
    axes[0].plot(time_proper_Myr, reference_pc, "--", label="analytic ODE reference")
    axes[0].set_xlabel("time [Myr]")
    axes[0].set_ylabel("radius [pc]")
    axes[0].legend()
    axes[1].plot(time_proper_Myr, velocity_kms, label="shell integrator")
    axes[1].plot(time_proper_Myr, reference_kms, "--", label="analytic ODE reference")
