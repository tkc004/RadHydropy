...
def main(config_filename=DEFAULT_CONFIG):
    ...
    active_boundary_proper_code = boundary_proper_code[
        config["par"]["mesh"]["ghost_cells"] : config["par"]["mesh"]["ghost_cells"]
        + config["par"]["mesh"]["grid_cells"]
        + 1
    ]
    radius_proper_code = 0.5 * (active_boundary_proper_code[1:] + active_boundary_proper_code[:-1])
    ...
    for axis, initial_values, final_values, ylabel in (
        (
            axes[0],
            initial.fluid.rho_radarray.value,
            final_snapshot.fluid.rho_radarray.value[interior],
            "density [code units]",
        ),
        (
            axes[1],
            initial.fluid.vel_radarray.value,
            final_snapshot.fluid.vel_radarray.value[interior],
            "radial velocity [code units]",
        ),
        (
            axes[2],
            initial.fluid.temp_radarray.value,
            final_snapshot.fluid.temp_radarray.value[interior],
            "temperature [code units]",
        ),
    ):
