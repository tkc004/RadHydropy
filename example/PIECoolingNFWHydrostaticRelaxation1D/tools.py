...
def build_initial_condition(config):
    ...
    writer.fluid.mu = np.full(grid_cells, initial["mu"])
    return writer
...
def analyze_snapshot(filename, config, halo, temperature_virial_unyt):
    ...
    gravity = (
        ...
        * mass_proper_cgs_g
        / np.maximum(radius_proper_cgs_cm, 1.0) ** 2
    )
    ...
    with open(filename, "w", encoding="utf-8") as report:
        ...
        report.writelines(
            "{:.8g} {:.8g} {:.8g} {:.8g} {:.8g} {:.8g} {:.8g} {}\n".format(
                ...
                row["minimum_temperature_proper_cgs_K"],
                row["atmosphere_mass_proper_Msun"],
                np.nanmax(np.abs(row["force_residual"])),
                temperature_floor,
                row["minimum_temperature_proper_cgs_K"] <= temperature_floor * 1.01,
            )
            for row in results
        )
