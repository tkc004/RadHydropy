...
from radhydropy.units import CodeUnits, time_seconds
...
def cosmic_mean_baryon_density(h0, omega_b, redshift):
    ...
    rho_critical = 3.0 * h0_cgs**2 / (8.0 * np.pi * unyt.physical_constants.gravitational_constant)
    return (float(omega_b) * rho_critical * (1.0 + float(redshift)) ** 3).to(unyt.g / unyt.cm**3)


def hubble_rate(h0, omega_m, omega_lambda, redshift):
    return h0 * np.sqrt(
        float(omega_m) * (1.0 + float(redshift)) ** 3 + float(omega_lambda),
    )
...
def _locate_shock(radius_proper_kpc, temperature_proper_cgs_K, virial_radius_proper_kpc):
    ...
    candidate = (radius_proper_kpc[:-1] > 0.5 * virial_radius_proper_kpc) & (
...
def rankine_hugoniot_diagnostics(filenames, config, halo):
    ...
    for snapshot_index in range(1, len(profiles) - 1):
        ...
        if dt_proper_myr <= 0.0:
            continue
        shock_speed = (
            (shock_positions[snapshot_index + 1] - shock_positions[snapshot_index - 1])
            / dt_proper_myr
            * 977.792221
        )
        ...
        rows.append(
            {
                ...
                "shock_radius_over_R200": shock_positions[snapshot_index]
                / virial_radius_proper_kpc,
                "shock_speed_proper_km_s": shock_speed,
                "mach_number_dimensionless": mach_number,
                "density_ratio_measured_dimensionless": density_downstream_proper_cgs_g_cm3
                / max(density_upstream_proper_cgs_g_cm3, 1.0e-99),
...
def write_rankine_hugoniot_report(rows, filename):
    header = (
        "time_proper_Myr shock_radius_proper_kpc shock_radius_over_R200 shock_speed_proper_km_s "
        "mach_number_dimensionless density_ratio_measured_dimensionless "
