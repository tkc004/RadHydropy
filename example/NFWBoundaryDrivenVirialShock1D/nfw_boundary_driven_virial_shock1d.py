    ...
    def SetBoundary(self, mesh, fluid, par):  # noqa: N802
        ...
        if hasattr(fluid, "ngamma_code"):
            ...
            right["ngamma_code"] = self._to_code_number_density(
                getattr(par.radiation, "hydrogen_ngamma_inflow", 0.0),
                scales,
            )
        self._copy_boundary_state(fluid, slice(0, first), left)
        self._copy_boundary_state(
            fluid,
            slice(right_start, right_start + par.mesh.ghost_cells),
            right,
        )
...
def _write_adiabatic_energy_audit(files, config, filename):
    ...
    def snapshot_energy(path):
        ...
        return (
            time_proper_cgs_s / float((1.0 * unyt.Myr).to_value(unyt.s)),
            float(np.sum(energy_cgs_erg)),
            boundary_energy_code,
            gravity_work_code,
        )
...
def main(config_filename=DEFAULT_CONFIG, adiabatic_only=False):
    ...
    config["par"]["simulation"]["initial_condition_filename"] = str(
        (
            config_filename.parent / config["par"]["simulation"]["initial_condition_filename"]
        ).resolve(),
    )
    ...
    adiabatic_audit = (
        Path(adiabatic_config["par"]["output"]["directory"])
        / "NFWBoundaryDrivenVirialShock1D_AdiabaticEnergyAudit.txt"
    )
    ...
    write_report(
        shock_history(
            ...
            halo,
            adiabatic_config,
            times_myr=adiabatic_times,
        ),
        ad_report,
    )
    write_report(
        shock_history(pie_files, halo, pie_config, times_myr=pie_times),
        pie_report,
    )
    plot_comparison(
        adiabatic_files,
        ...
        figure,
        pie_config,
        adiabatic_times_myr=adiabatic_times,
        pie_times_myr=pie_times,
    )
