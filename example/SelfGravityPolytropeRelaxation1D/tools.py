...
"""Analytic and I/O helpers for the self-gravitating n=1 polytrope."""  # noqa: CPY001
...
def equilibrium_density(
    ...
    radius_dimensionless = np.asarray(
        radius_proper_cgs_cm_unyt / radius_polytropic_cgs_cm_unyt,
        dtype=float,
    )
...
def equilibrium_temperature(
    ...
    return (
        pre_proper_cgs_erg_cm3_unyt
        * mu_dimensionless
        * PROTON_MASS_CGS
        * (unyt.g)
        / (rho_proper_cgs_g_cm3_unyt * BOLTZMANN_CONSTANT_CGS * (unyt.erg / unyt.K))
    ).to(unyt.K)
...
def build_initial_condition(config):
    ...
    temperature_proper_unyt = equilibrium_temperature(
        rho_proper_cgs_g_cm3_unyt,
        k_poly_cgs,
        initial["mu_dimensionless"],
    )
    radius_fraction_dimensionless = radius_proper_cgs_cm_unyt / initial["radius_polytropic_proper"]
    velocity_proper_code_unyt = (
        float(initial["vel_perturbation_dimensionless"])
        * np.asarray(
            radius_fraction_dimensionless,
            dtype=float,
        )
