...
def reference_step(
    ...
    radius_new_comoving_code = (
        radius_comoving_code + dt_supercomoving_code * vel_half_supercomoving_code
    )
    return (
        radius_new_comoving_code,
        vel_half_supercomoving_code
...
def analytic_turnaround(
    ...
    vel_proper_code = (
        hubble_initial_code * radius_proper_code + vel_supercomoving_code / a_initial_dimensionless
    )
    ...
    solution = solve_ivp(
        rhs,
        (time_cosmic_code, cosmology.t_ref),
        (radius_proper_code, vel_proper_code),
        rtol=1.0e-11,
        atol=1.0e-11,
...
def run_case(
    ...
    vel_supercomoving_code = (
        -(ai_code**2) * hubble_initial_code * overdensity_dimensionless * radius_comoving_code / 3.0
    )
    shell = DarkMatterShells(
        [radius_comoving_code],
        [vel_supercomoving_code],
        [mass_comoving_code],
        fixed_enclosed_mass=mass_comoving_code,
    ...
    while tau_supercomoving_code < tau_final_supercomoving_code - 1.0e-14:
        ...
        shell.step(
            ...
            background_enclosed_mass=lambda radius_shell_comoving_code: (
                4.0 * np.pi / 3.0 * rho_comoving_code * np.asarray(radius_shell_comoving_code) ** 3
            ),
...
def make_comparison(
    ...
    for label, code_class, omega_m, omega_lambda, final_scale_factor in cases:
        ...
        error_axis.plot(
            time_cosmic_code,
            np.maximum(np.abs(history[:, 3] - history[:, 4]), 1.0e-18),
...
def run():
    ...
    make_comparison(
        100.0,
        "CosmologicalTopHatEdSVsLCDM_1e12Msun_delta0p02.jpg",
        initial_overdensity=0.02,
