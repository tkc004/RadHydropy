...
def run_simulation(config):
    ...
    return (
        ...
        initial_energy,
        initial_internal,
        np.asarray(source_times),
        np.asarray(source_momenta),
        np.asarray(source_energies),
        np.asarray(source_works),
    )
...
def main(config_filename=CONFIG):
    ...
    # exchange energy.
    expected_work = 0.5 * (expected_momentum**2 - initial_momentum**2) / mass
    ...
    final_internal = (
        float(sim.fluid.InternalEnergy_code[first])
        if hasattr(sim.fluid, "InternalEnergy_code")
        else initial_internal
    )
    ...
    plotted = np.unique(np.r_[np.arange(0, len(source_times), 10), len(source_times) - 1])
    axes[0].plot(time_proper_code, expected_momentum, "--", label="analytic")
    axes[0].plot(source_times[plotted], source_momenta[plotted], ":o", markersize=4, label="Rsim")
    axes[0].set_ylabel("radial momentum")
    axes[1].plot(time_proper_code, expected_energy, "--", label="analytic")
    axes[1].plot(source_times[plotted], source_energies[plotted], ":o", markersize=4, label="Rsim")
    axes[1].set_ylabel("total energy")
    axes[2].plot(time_proper_code, expected_work, "--", label="analytic")
    axes[2].plot(source_times[plotted], source_works[plotted], ":o", markersize=4, label="Rsim")
