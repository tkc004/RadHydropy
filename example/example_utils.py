...
def snapshot_physical_fields(hdf5_filename, config):
    ...
    if par.supercomoving_coordinates:
        ...
        radius_comoving_code = 0.5 * (boundary_comoving_code[:-1] + boundary_comoving_code[1:])
        density_comoving_code = np.asarray(
            fluid.rho_comoving_code[first:last],
            dtype=float,
        )
        velocity_supercomoving_code = np.asarray(
            fluid.vel_supercomoving_code[first:last],
            dtype=float,
        )
        temperature_supercomoving_code = np.asarray(
            fluid.temp_supercomoving_code[first:last],
            dtype=float,
        )
    ...
    boundary_proper_code = np.asarray(
        mesh.boundary_proper_code[first : last + 1],
        dtype=float,
    )
    return {
...
def write_radial_profile_csv(hdf5_filename, config, csv_filename=None):
    ...
    csv_filename = hdf5_filename.with_suffix(".csv") if csv_filename is None else Path(csv_filename)
