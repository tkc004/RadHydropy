...
def analytic_density_profile(radius_proper_code, time_proper_code, config, cell_faces=None):
    ...
    if cell_faces is None:
        inside = (radius_proper_code >= injection_radius_proper_code) & (
            radius_proper_code <= front_radius_proper_code
        )
        profile[inside] = (
            density_outflow_proper_code
            * (injection_radius_proper_code / radius_proper_code[inside]) ** 2
        )
    ...
    profile[inside] = (
        3.0
        * density_outflow_proper_code
        * injection_radius_proper_code**2
        * (right[inside] - left[inside])
        / volume_factor[inside]
    )
    return profile, front_radius_proper_code
