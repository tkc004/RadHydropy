...
class BertschingerBoundarySolver(Solver):
    ...
    def SetBoundary(self, mesh, fluid, par):  # noqa: N802
        ...
        fluid.vel_supercomoving_code[right] = (
            -(scale_factor**2) * hubble * delta * radius_comoving_code / 3.0
        )
...
def build_initial_condition(config):
    ...
    temp_supercomoving_code = (
        ...
        * mean_molecular_weight
        / (
            ...
            * code_units.boltzmann_code
            / code_units.proton_mass_code
        )
    )
    ...
    sim.dark_matter = DarkMatterShells(
        ...
        np.zeros(1),
        np.full(1, 1.0e-30) * code_units.mass_unit,
...
def _similarity_profiles(sim, solution):
    ...
    rho_proper_code = cosmology.physical_density(
        np.asarray(sim.fluid.rho_comoving_code[interior], dtype=float),
        tau,
    )
    vel_peculiar_proper_code = cosmology.physical_velocity(
        np.asarray(sim.mesh.x_comoving_code[interior], dtype=float),
        np.asarray(sim.fluid.vel_supercomoving_code[interior], dtype=float),
        tau,
    )
    pre_proper_code = cosmology.physical_pressure(
        np.asarray(sim.fluid.pre_supercomoving_code[interior], dtype=float),
        tau,
        sim.par.hydrodynamics.gamma,
    )
    boundaries = (
        np.asarray(
            sim.mesh.boundary_comoving_code[
                sim.par.mesh.ghost_cells : sim.par.mesh.ghost_cells + sim.par.mesh.grid_cells + 1
            ],
            dtype=float,
        )
        * scale_factor
    ...
    shell_mass_comoving_code = (
        rho_proper_code * (4.0 * np.pi / 3.0) * (boundaries[1:] ** 3 - boundaries[:-1] ** 3)
    )
...
def _plot_comparison(numerical, reference, output, numerical_label):
    ...
    axes[3].loglog(
        ...
        reference.mass_in,
        "--",
        label="standalone interior",
    )
    axes[3].loglog(
        reference.lambda_out,
        reference.mass_out,
        "--",
        label="standalone exterior",
    )
...
def main(config_filename=DEFAULT_CONFIG):
    ...
    sim.par.gravity = Gravity(
        ...
        dark_matter=initial.dark_matter,
    ...
    initial_output = (
        Path(output["directory"]) / "BertschingerGasReference_RadHydroInitialCondition.jpg"
    )
    _plot_comparison(initial_numerical, reference, initial_output, "RadHydro IC")
    comparison_output = (
        Path(output["directory"]) / "BertschingerGasReference_RadHydroComparison.jpg"
    )
