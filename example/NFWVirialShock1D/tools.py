...
def rankine_hugoniot_diagnostics(filenames, config):
    ...
    for snapshot_index in range(1, len(profiles) - 1):
        ...
        sound_speed = (
            np.sqrt(
                gamma * 1.380649e-16 * temperature_upstream_proper_cgs_K / (mu * 1.67262192369e-24),
            )
            / 1.0e5
        )
        ...
        rows.append(
            {
                ...
                "temperature_ratio_measured_dimensionless": temperature_downstream_proper_cgs_K
                / max(temperature_upstream_proper_cgs_K, 1.0e-99),
