...
def main(config_filename=CONFIG):
    ...
    for row, (radius_comoving_code, mass_comoving_code) in enumerate(
        ...
        rho_comoving_code = np.divide(
            mass_comoving_code,
            shell_volume,
            out=np.full_like(mass_comoving_code, np.inf),
        ...
        bin_index = np.clip(
            np.digitize(radius_comoving_code, bin_edges) - 1,
            0,
            bin_edges.size - 2,
        )
    ...
    np.savez(
        data_file,
        time_cosmic_code=times,
        radius_comoving_code=radii,
    ...
    axes[0].set(
        xlabel="proper radius [kpc]",
        ylabel="DM density / background",
        title="Gas-free DM density contrast",
    )
    axes[1].set(
        xlabel="proper radius [kpc]",
        ylabel="enclosed DM mass [code mass]",
