...
class CosmologicalVirialShockPhysics:
    ...
    def __init__(
        ...
        self.transition_redshift = (
            ...
            else float(transition_redshift)
        )
    ...
    def update_cosmic_boundary(self, time_cosmic_code):
        ...
        temperature_initial_cgs_K = (
            self.cmb_temperature_0_cgs_K / self.initial_a
        )
        temperature_physical = temperature_initial_cgs_K * (
            self.initial_a / scale_factor
        ) ** 2
        ...
        self.sim.par.boundary.temperature_inflow_proper = (
            temperature_physical * scale_factor**2
        )
        ...
        self.sim.par.hydro_temperature_floor = (
            ...
            else self.minimum_temperature * scale_factor**2
        )
    ...
    def preserve_outer_background_cell(self):
        ...
        pre_supercomoving_code = float(np.asarray(
            self.sim.fluid.eos.pressure(
                rho_comoving_code, temperature_supercomoving_code, mu
            ),
            dtype=float,
        ))
        ...
        self.sim.fluid.Mass_code[index] = (
            rho_comoving_code * volume_comoving_code
        )
        self.sim.fluid.Mom_code[index] = (
            rho_comoving_code * vel_supercomoving_code * volume_comoving_code
        )
        self.sim.fluid.Energy_code[index] = float(np.asarray(
