...
class CosmologicalVirialShockDiagnostics:
    ...
    def on_snapshot(self, sim, snapshot_filename, output_index):
        ...
        gas_profile["temperature_proper_cgs_K"] = (
            np.asarray(sim.fluid.temp_supercomoving_code[first:last], dtype=float)
            / scale_factor**2
        )
        ...
        radius_record = et.profiles(
            sim, self.dark_matter, time_cosmic_code, self.config,
        ...
        gas_mass_comoving_code = (
            ...
            * (4.0 * np.pi / 3.0) * np.diff(gas_edges**3)
        )
        ...
        for source_key, report_key in shock_keys:
            radius_record[report_key] = (
                ...
                if 0 <= shock_index < int(sim.par.mesh.grid_cells) else np.nan
            )
        ...
        crossing = {
            "entered_cells": int(np.count_nonzero(current_halo_mask & ~previous)) if previous is not None else 0,
            "exited_cells": int(np.count_nonzero(previous & ~current_halo_mask)) if previous is not None else 0,
        }
        for key in (
            "total_energy_code", "thermal_energy_code", "kinetic_energy_code",
            "gravitational_work", "hydro_energy_change",
            "thermochemistry_energy_change", "compression_work", "shock_work",
        ):
            if previous is None:
                crossing[key] = 0.0
            else:
    ...
    def on_step(
        ...
        energy_change = (
            audit_state["total_gas_energy_code"] - previous_energy
        )
    ...
    def finalize(self, sim):
        ...
        plot_specific_angular_momentum_evolution = context["plot_specific_angular_momentum_evolution"]
        ...
        plot_baryon_normalized_density_comparison = context["plot_baryon_normalized_density_comparison"]
