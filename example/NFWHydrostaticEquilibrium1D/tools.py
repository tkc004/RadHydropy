...
def nfw_halo_parameters(
    ...
    virial_radius = (3.0 * halo_mass_proper_g / (4.0 * np.pi * overdensity * rho_critical)) ** (
        1.0 / 3.0
    )
...
def nfw_enclosed_mass(radius_proper_unyt, halo):
    ...
    return halo["mass_halo_proper_g_unyt"] * ((np.log1p(x) - x / (1.0 + x)) / shape)
...
def hydrostatic_density_profile(
    ...
    shell_volume = (
        ...
        * np.pi
        / 3.0
        * (
            boundaries_proper_cgs_cm_unyt[1:].value ** 3
            - boundaries_proper_cgs_cm_unyt[:-1].value ** 3
        )
    )
...
def read_and_plot(outfilename, config, halo, temperature_proper_unyt, figure_filename):
    ...
    rho_expected_proper_cgs_g_cm3 = quantity_to_value(
        rho_expected_proper_cgs_g_cm3_unyt,
        unyt.g / unyt.cm**3,
    )
