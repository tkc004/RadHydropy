"""Fixed-radiation hydrogen photoionization box without recombination.
...
radiation-field evolution, recombination, collisional ionization, and thermal
source update are disabled. The run stops once the gas is 99 percent ionized
and writes a JPG comparing the neutral fraction against the exponential
fixed-field solution.
"""
...
runparams = {
    'simname': 'HydrogenPhotoionizationNoRecombination1D',
...
def analytic_neutral_fraction(time_yr):
    ...
    return ICparams['xHIini'] * np.exp(-rate_photo * time.to_value(unyt.s))
...
def main():
    ...
    figure_filename = rundir + '/HydrogenPhotoionizationNoRecombination1D.jpg'
    save_history_plot(history, figure_filename)

    print('Hydrogen photoionization no-recombination example finished')
