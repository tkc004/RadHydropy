...
class Simwrap():
    def __init__(self) -> None:
        ...
        #self.mesh.boundary = np.linspace(self.par.boxsize[0]*0.1,self.par.boxsize[0]+0.5*dx,self.par.nogrid+1)
...
def ReadandPlot(outfilename,**kwargs):
    ...
    x = np.linspace(ICparams["rinj"],ICparams["rinj"]+ICparams["boxsize"],ICparams["nogrid"]) 
    rhoana = runparams['rho_outflow']*ICparams['rinj']**2/x**2
