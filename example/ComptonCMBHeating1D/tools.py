...
def build_initial_condition(config):
    ...
    result.par.simulation = SimpleNamespace(
        time_code=initial['current_time'], box_size=initial['box_size'],
    ...
    result.fluid.rho_code = density_proper_cgs_g_cm3_unyt
    result.fluid.vel_code = np.zeros(result.par.mesh.grid_cells, dtype=float)
