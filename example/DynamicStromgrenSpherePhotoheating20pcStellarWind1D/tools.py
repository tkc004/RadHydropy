...
def build_static_problem(config):
    ...
    mesh.boundary_proper_code = quantity_to_value(
        boundary_proper_cgs_cm_unyt,
        par.units.CodeUnits.length_unit,
    )
    ...
    mesh.x_proper_code = (
        0.75
        * (mesh.boundary_proper_code[1:] ** 4 - mesh.boundary_proper_code[:-1] ** 4)
        / (mesh.boundary_proper_code[1:] ** 3 - mesh.boundary_proper_code[:-1] ** 3)
    )
    ...
    if wind_cells > 0:
        ...
        injection_radius_proper_code = quantity_to_value(
            example["radius_injection_proper"],
            par.units.CodeUnits.length_unit,
        )
        wind_density_proper_code = quantity_to_value(
            _wind_density(config),
            par.units.CodeUnits.density_unit,
        )
        wind_velocity_proper_code = quantity_to_value(
            example["wind_velocity_proper"],
            par.units.CodeUnits.velocity_unit,
        )
        wind_temperature_proper_code = quantity_to_value(
            example["wind_temperature_proper"],
            par.units.CodeUnits.temperature_unit,
        )
        fluid.rho_proper_code[active_slice] = (
            wind_density_proper_code * (injection_radius_proper_code / cell_center_proper_code) ** 2
        )
