...
def run(config_filename=DEFAULT_CONFIG, riemann_solver=None):
    ...
    while float(sim.fluid.time_proper_code) < target_proper_code:
        ...
        sim.solver.SetInterFaceFlux(
            sim.mesh,
            sim.fluid,
            config["par"]["boundary"]["condition"],
            method=config["par"]["hydrodynamics"]["riemann_solver"],
            order=0,
        )
        fluxes.append(
            [
    ...
    data = {
        "time_proper_code": np.array([s[0] for s in snapshots]),
        "radius_proper_code": radius_proper_code,
        "rho_proper_code": np.array([s[2] for s in snapshots]),
        "vel_proper_code": np.array([s[3] for s in snapshots]),
        "pre_proper_code": np.array([s[4] for s in snapshots]),
        "temp_proper_code": np.array([s[5] for s in snapshots]),
        "entropy_dimensionless": np.array([s[6] for s in snapshots]),
        "flux_proper_code": np.asarray(fluxes),
    }
