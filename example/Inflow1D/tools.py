...
def build_initial_condition(config):
    ...
    sim.mesh.boundary_proper_code = as_named_array(np.linspace(
        0.0, quantity_to_value(initial['box_size'], code_units.length_unit),
        grid_cells + 1,
    ))
    sim.fluid.vel_proper_code = as_named_array(np.full(
        grid_cells, quantity_to_value(initial['velocity'], code_units.velocity_unit)
    ))
    sim.fluid.temp_proper_code = as_named_array(np.full(
        grid_cells, quantity_to_value(initial['temperature'], code_units.temperature_unit)
    ))
