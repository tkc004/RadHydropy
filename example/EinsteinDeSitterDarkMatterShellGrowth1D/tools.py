...
def make_shells(config, overdensity=None):
    ...
    rmin = float(initial_condition['radius_inner_dimensionless'])
    rmax = float(initial_condition['radius_outer_dimensionless'])
    ...
    inside = radius < float(initial_condition['radius_top_hat_comoving'])
