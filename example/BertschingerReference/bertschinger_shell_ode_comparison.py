...
def run_comparison(config_filename=DEFAULT_CONFIG):
    ...
    while tau < tau_comparison_final_supercomoving - 1.0e-12:
        ...
        if caustic is not None and np.isfinite(caustic):
            caustic_values.append((np.log(time_cosmic_code / initial_time), caustic / turnaround))
    ...
    axis.plot(ode.xi, ode.lam, color="black", linewidth=2.0, label="Bertschinger Eq. (4.1) ODE")
    if caustic_values:
    ...
    if apocentre_events.size and len(turnaround_values) >= 2:
        ...
        for lower, upper in zip(ta_history[:-1, 0], ta_history[1:, 0], strict=False):
            ...
            bins.append(
                (
                    np.log(0.5 * (lower + upper) / initial_time),
                    np.median(values),
    ...
    if len(caustic_values) or slope_profiles or len(apocentre_values):
        ...
        comparison_figure.savefig(
            Path(config["par"]["output"]["directory"]) / "BertschingerSplashbackComparison.jpg",
            dpi=200,
        )
        plt.close(comparison_figure)
        np.savez(
            Path(config["par"]["output"]["directory"]) / "BertschingerSplashbackComparison.npz",
            **comparison_data,
        )
