...
def _run_case(
    ...
    initial.write(case_config["par"]["simulation"]["initial_condition_filename"], validate=True)
    ...
    return {
        ...
        "rho_proper_cgs_g_cm3": (
            ...
            * PROTON_MASS_G
            / case_config["par"]["thermochemistry"]["hydrogen_mass_fraction"]
        ),
...
def _shock_diagnostics(result, table, config):
    ...
    right = (centers_proper_cgs_cm > center_proper_cgs_cm) & (
    ...
    if result["adiabatic"]:
        ...
        cooling_length = cooling_length_estimate(
            ...
            post_temperature,
            post_density,
            float(thermo["hydrogen_mass_fraction"]),
            mu,
            gamma,
            result["metallicity"],
            float(thermo["metal_pie_redshift"]),
            post_velocity,
        )
    final_boundary_proper_cgs_cm = snapshot["boundary_proper_cgs_cm"]
    final_centers_proper_cgs_cm = 0.5 * (
        final_boundary_proper_cgs_cm[1:] + final_boundary_proper_cgs_cm[:-1]
    )
    ...
    result.update(
        {
            "snapshot": snapshot,
            "shock_radius_cgs_cm": centers_proper_cgs_cm[shock_index]
...
def _plot(results, filename):
    ...
    for index, result in enumerate(results):
        ...
        x_kpc = (
            0.5
            * (data["boundary_proper_cgs_cm"][1:] + data["boundary_proper_cgs_cm"][:-1])
            / KPC_CM
        )
        ...
        axes[1, 1].plot(
            x_kpc,
            data["temperature_proper_cgs_K"] / result["post_temperature_cgs_K"],
            style,
            color=color,
            label=label,
        )
    for result in results:
        ...
        if np.isfinite(measured_kpc):
            axes[1, 1].plot(
                [shock_kpc - measured_kpc, shock_kpc],
                [0.035, 0.035],
                color=color,
                lw=3.0,
                solid_capstyle="butt",
