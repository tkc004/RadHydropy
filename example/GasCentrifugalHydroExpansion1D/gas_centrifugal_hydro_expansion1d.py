...
def build_initial_condition(config):
    ...
    boundary_proper_unyt = initial_condition["radius_inner_proper"] + boundary_proper_unyt * (
        initial_condition["radius_outer_proper"] - initial_condition["radius_inner_proper"]
    )
...
def run_simulation(config):
    ...
    specific_angular_momentum_proper_code = np.asarray(
        initial.simulation.fluid.specific_angular_momentum_code,
        dtype=float,
    ).copy()
    ...
    return (
        ...
        initial_energy,
        initial_radius,
        float(sim.cumulative_gravity_work),
...
def main(config_filename=CONFIG):
    ...
    saved_total_energy = np.sum(saved_energy) - np.sum(central_mass * saved_mass / saved_radius)
    ...
    axes[3].semilogy(
        saved_radius,
        np.maximum(pressure_ratio, np.finfo(float).tiny),
        ":o",
        markersize=4,
        label="Rsim pressure support",
    )
    axes[3].axhline(1.0, color="k", linestyle=":")
    axes[3].set_ylabel("thermal / dynamical scale")
    axes[4].bar(
        ["ODE", "Rsim"],
        [ode_total_energy, saved_total_energy],
        color=["C0", "C1"],
    )
