...
def _plot(histories, config, filename):
    ...
    ax.plot(time_proper_Myr.to_value(unyt.Myr), analytic, "k-", lw=2.0, label="Analytic $R_I(t)$")
...
def main(config_filename=Path(__file__).with_name("static_stromgren_c2ray_comparison.yaml")):
    ...
    for steps in example["comparison_instantaneous_steps"]:
        ...
        histories[f"instantaneous_{steps}"] = _run_case(
            nested,
            tools,
            f"instantaneous_{steps}",
            "instantaneous",
            steps,
            root,
        )
    figure = (
        Path(nested["par"]["output"]["directory"]) / "StaticStromgrenC2RayComparison_IFront.jpg"
    )
    summary = (
        Path(nested["par"]["output"]["directory"]) / "StaticStromgrenC2RayComparison_IFront.csv"
    )
