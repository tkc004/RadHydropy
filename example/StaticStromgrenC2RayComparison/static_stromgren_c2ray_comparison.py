...
def _plot(histories, config, filename):
    ...
    time_proper_Myr = np.linspace(0.0, config['par']['simulation']['final_time'].to_value(unyt.Myr), 1200) * unyt.Myr
    analytic = sa.ionization_front_radius(
        time_proper_Myr, example['source_photon_rate'], initial['hydrogen_number_density'],
