...
def _refresh_mesh_geometry(mesh, config):
    ...
    if par.simulation.coordinate_system == "cartesian":
        ...
        mesh.volume_proper_code = (
            np.absolute(mesh.boundary_proper_code[1:] ** 3 - mesh.boundary_proper_code[:-1] ** 3)
            * 4.0
            * np.pi
            / 3.0
        )
...
def ionization_front_position(mesh, fluid, config, neutral_fraction=0.5):
    ...
    return radius_proper_cgs_kpc_unyt[left] + weight * (
        radius_proper_cgs_kpc_unyt[right] - radius_proper_cgs_kpc_unyt[left]
    )
