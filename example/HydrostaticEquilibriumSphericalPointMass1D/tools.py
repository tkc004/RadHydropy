...
def point_mass_hydrostatic_density_profile(
    ...
    if hasattr(x_proper_code, "to_value"):
        ...
        coord_value = (
            np.asarray(x_proper_code, dtype=float)
    ...
    if hasattr(point_mass, "to_value"):
        ...
        point_mass_value = (
            np.asarray(point_mass, dtype=float) * code_unit_scales(code_unit_system)["mass_g"]
        )
    else:
    ...
    if hasattr(rho_reference_proper, "to_value"):
        ...
        rho_value = (
            np.asarray(rho_reference_proper, dtype=float)
...
def point_mass_acceleration(
    ...
    if hasattr(point_mass_proper_g, "to_value"):
        ...
        point_mass_proper_g = (
            np.asarray(point_mass_proper_g, dtype=float)
            * code_unit_scales(code_unit_system)["mass_g"]
        )
    else:
    ...
    if hasattr(softening_proper_unyt, "to_value"):
        ...
        softening_proper_cgs_cm = (
            np.asarray(softening_proper_unyt, dtype=float)
            * code_unit_scales(code_unit_system)["length_cgs_cm"]
        )
    else:
...
def plot_snapshot(outfilename, config, **kwargs):
    ...
    code_units_obj = (
        CodeUnits.from_mapping(code_units_mapping) if code_units_mapping is not None else None
    )
    ...
    x_units = getattr(
        coordinate_proper_code,
        "units",
        code_units_obj.length_unit.units if code_units_obj is not None else unyt.cm,
    )
    rho_units = getattr(
        rho_num,
        "units",
        code_units_obj.density_unit.units if code_units_obj is not None else unyt.g / unyt.cm**3,
    )
    vel_units = getattr(
        vel_code_num,
        "units",
        code_units_obj.velocity_unit.units if code_units_obj is not None else unyt.cm / unyt.s,
    )
