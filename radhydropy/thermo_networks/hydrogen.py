...
def _require_numeric_array(value: Any, label: str) -> Any:
    if hasattr(value, "to_value"):
        raise TypeError(f"{label} must be a plain numeric array, not a unyt quantity")
    return np.asarray(value, dtype=float)


def _canonical_fluid_primitive_arrays(fluid: Any, par: Any) -> Any:
...
def _canonical_mesh_geometry_arrays(mesh: Any, par: Any) -> Any:
...
def _optional_numeric_value(value: Any, unit: Any, default: Any = None) -> Any:
    if value is None:
        if default is None:
            return None
...
def _cgs_alpha_B(temperature_cgs_K: Any) -> Any:  # noqa: N802, N803
    ...
    if np.any(valid):
        ...
        result[valid] = 2.753e-14 * lam**1.5 * (1.0 + (lam / 2.740) ** 0.407) ** -2.242
    return result


def cgs_alpha_B(temperature_cgs_K: Any) -> Any:  # noqa: N802, N803
    """Return the case-B recombination coefficient in cgs units."""
    return _cgs_alpha_B(temperature_cgs_K)


def _cgs_alpha_A(temperature_cgs_K: Any) -> Any:  # noqa: N802, N803
    ...
    if np.any(valid):
        lam = 315614.0 / temperature_cgs_K[valid]
        result[valid] = 1.269e-13 * lam**1.503 * (1.0 + (lam / 0.522) ** 0.470) ** -1.923
    return result


def _cgs_beta(temperature_cgs_K: Any) -> Any:  # noqa: N803
    temperature_cgs_K = np.asarray(temperature_cgs_K, dtype=float)
    result = np.zeros_like(temperature_cgs_K, dtype=float)
    valid = temperature_cgs_K > 0.0
    if np.any(valid):
        ...
        temp5 = temp / 1.0e5
        result[valid] = 1.17e-10 * temp**0.5 * np.exp(-157809.1 / temp) / (1.0 + temp5**0.5)
    return result


def collisional_equilibrium_neutral_fraction(temperature_cgs_K: Any) -> Any:  # noqa: N803
...
def _cgs_gamma_line_eHI(temperature_cgs_K: Any) -> Any:  # noqa: N802, N803
    temperature_cgs_K = np.asarray(temperature_cgs_K, dtype=float)
    result = np.zeros_like(temperature_cgs_K, dtype=float)
    valid = temperature_cgs_K > 0.0
    if np.any(valid):
        temp = temperature_cgs_K[valid]
        temp5 = temp / 1.0e5
        result[valid] = 7.5e-19 * np.exp(-118348.0 / temp) / (1.0 + temp5**0.5)
    return result


def _cgs_gamma_ion_eHI(temperature_cgs_K: Any) -> Any:  # noqa: N802, N803
    temperature_cgs_K = np.asarray(temperature_cgs_K, dtype=float)
    result = np.zeros_like(temperature_cgs_K, dtype=float)
    valid = temperature_cgs_K > 0.0
    if np.any(valid):
        temp = temperature_cgs_K[valid]
        temp5 = temp / 1.0e5
        result[valid] = 2.54e-21 * temp**0.5 * np.exp(-157809.1 / temp) / (1.0 + temp5**0.5)
    return result


def _cgs_gamma_ff_eHII(temperature_cgs_K: Any) -> Any:  # noqa: N802, N803
    temperature_cgs_K = np.asarray(temperature_cgs_K, dtype=float)
    result = np.zeros_like(temperature_cgs_K, dtype=float)
    valid = temperature_cgs_K > 0.0
    if np.any(valid):
        temp = temperature_cgs_K[valid]
        result[valid] = (
            1.42e-27 * temp**0.5 * (1.1 + 0.34 * np.exp(-((5.5 - np.log10(temp)) ** 2) / 3.0))
        )
    return result


def _cgs_gamma_B_eHII(temperature_cgs_K: Any) -> Any:  # noqa: N802, N803
    temperature_cgs_K = np.asarray(temperature_cgs_K, dtype=float)
    result = np.zeros_like(temperature_cgs_K, dtype=float)
    valid = temperature_cgs_K > 0.0
    if np.any(valid):
        temp = temperature_cgs_K[valid]
        lam = 315614.0 / temp
        result[valid] = 3.435e-30 * temp * lam**1.970 * (1.0 + (lam / 2.250) ** 0.376) ** -3.720
    return result


def _cgs_gamma_A_eHII(temperature_cgs_K: Any) -> Any:  # noqa: N802, N803
    ...
    if np.any(valid):
        temp = temperature_cgs_K[valid]
        lam = 315614.0 / temp
        result[valid] = 1.778e-29 * temp * lam**1.965 * (1.0 + (lam / 0.541) ** 0.502) ** -2.697
    return result


def cgs_hydrogen_number_density(
    rho_cgs_g_cm3: Any,
    hydrogen_mass_fraction: Any = 1.0,
) -> Any:
    """Return hydrogen number density in cgs units."""
    return hydrogen_mass_fraction * np.asarray(rho_cgs_g_cm3, dtype=float) / PROTON_MASS_CGS


def _cgs_hydrogen_number_density(
    rho_cgs_g_cm3: Any,
    hydrogen_mass_fraction: Any = 1.0,
) -> Any:
    """Backward-compatible private alias for the public conversion function."""
    return cgs_hydrogen_number_density(rho_cgs_g_cm3, hydrogen_mass_fraction)


def _cgs_photoionization_frequency(
    ngamma_cgs_cm3: Any,
    sigma_gamma_cgs_cm2: Any,
) -> Any:
...
def cgs_photoionization_frequency(
    ngamma_cgs_cm3: Any,
    sigma_gamma_cgs_cm2: Any,
) -> Any:
    """Return the photoionization frequency in cgs units."""
    return _cgs_photoionization_frequency(ngamma_cgs_cm3, sigma_gamma_cgs_cm2)
...
def _cgs_source_thermal_rate(
    ...
    hydrogen_mass_fraction: Any = 1.0,
    *,
    recombination: Any = True,
    collisional_ionization: Any = True,
    atomic_cooling: Any = True,
    ngamma_cgs_cm3: Any = None,
    sigma_gamma_cgs_cm2: Any = 1.0,
    epsilon_gamma_cgs_erg: Any = 0.0,
    compton_cmb_enabled: Any = False,
    compton_cmb_redshift: Any = 0.0,
    cmb_temperature_0_cgs_K: Any = 2.7255,  # noqa: N803
) -> Any:
    ...
    if ngamma_cgs_cm3 is None:
        ...
        photoheating_cgs_erg_s_per_atom = (
            SPEED_OF_LIGHT_CGS * epsilon_gamma_cgs_erg * sigma_gamma_cgs_cm2 * ngamma_cgs_cm3
        )
...
def _cgs_static_neutral_fraction_rate(
    ...
    *,
    ...
    ngamma_cgs_cm3: Any = None,
    sigma_gamma_cgs_cm2: Any = 1.0,
    recombination_coefficient_cgs_cm3_s: Any = None,
    ionization_coefficient_cgs_cm3_s: Any = None,
) -> Any:
    xHI = np.clip(np.asarray(xHI, dtype=float), 0.0, 1.0)
    ionized = 1.0 - xHI
    nH = _cgs_hydrogen_number_density(rho_cgs_g_cm3, hydrogen_mass_fraction)
...
def _cgs_static_neutral_fraction_implicit_update(
    ...
    *,
    ...
    b = -(
        1.0
        + dt_value * (photoionization_rate_cgs_s + 2.0 * recombination_rate_s + ionization_rate_s)
    )
...
def interior_slice(par: Any) -> Any:
...
def source_state(mesh: Any, fluid: Any, par: Any) -> Any:
    ...
    from radhydropy.thermo_networks.hydrogen_source_state import build_source_state  # noqa: PLC0415
    ...
    return build_source_state(
        ...
        cgs_source_state_from_code=cgs_source_state_from_code,
        canonical_fluid_primitive_arrays=_canonical_fluid_primitive_arrays,
        canonical_mesh_geometry_arrays=_canonical_mesh_geometry_arrays,
        fast_source_scaling=_fast_source_scaling,
        optional_numeric_value=_optional_numeric_value,
        parameter_value=_parameter_value,
        interior_slice=interior_slice,
    )


def trace_spherical_tau(
    mesh: Any,
    rho: Any,
    xHI: Any,
    hydrogen_mass_fraction: Any,
    sigma_gamma: Any,
) -> Any:  # noqa: N803
    ...
    boundary_runtime_code = mesh.geometry_state.boundary_proper_code
...
def get_timestep(
    ...
    *,
    ...
    ionization_result = _ionization_timestep(state, ngamma_cgs_cm3, source_CFL, verbose)
    if ionization_result is not None:
        neutral_dt, neutral_detail = ionization_result
        candidates.append(neutral_dt)
        if neutral_detail is not None:
            debug_lines.append(neutral_detail)
    if state["thermal_coupling"]:
        source_thermal_rate = thermal_rate(state, ngamma_cgs_cm3)
        thermal_result = _thermal_timestep(state, source_thermal_rate, source_CFL, verbose)
        if thermal_result is not None:
            thermal_dt, thermal_detail = thermal_result
            candidates.append(thermal_dt)
            if thermal_detail is not None:
                debug_lines.append(thermal_detail)
...
def _ionization_timestep(
    ...
    enabled = (
        state["recombination"] or state["collisional_ionization"] or ngamma_cgs_cm3 is not None
    )
    if not enabled:
        return None
    ...
    if not np.any(neutral_mask):
        return None
    valid_cells = np.where(valid)[0][neutral_mask]
    neutral_times = neutral_times[neutral_mask]
    index = int(np.argmin(neutral_times))
    neutral_cell = int(valid_cells[index])
...
def _thermal_timestep(
    state: Any,
    source_thermal_rate: Any,
    source_CFL: Any,
    verbose: Any,
) -> Any:
    active = np.asarray(
    ...
    valid = active & (np.abs(dudt) > 0.0) & (state["specific_energy_cgs_erg_g"] > 0.0)
    if not np.any(valid):
        return None
    with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
        thermal_times = state["specific_energy_cgs_erg_g"][valid] / np.abs(dudt[valid])
    thermal_mask = np.isfinite(thermal_times) & (thermal_times > 0.0)
    if not np.any(thermal_mask):
        return None
    thermal_cells = np.where(valid)[0][thermal_mask]
    thermal_times = thermal_times[thermal_mask]
    index = int(np.argmin(thermal_times))
    thermal_cell = int(thermal_cells[index])
...
def _fast_source_scaling(fluid: Any, par: Any, gamma: Any) -> Any:
...
def _rotational_specific_energy_code(mesh: Any, fluid: Any, par: Any) -> Any:
...
def _fast_source_state(mesh: Any, fluid: Any, par: Any) -> Any:
    ...
    velocity_supercomoving_cgs_cm_s = (
        np.asarray(vel_runtime_code[interior], dtype=float) * unit_conversion["velocity_cgs_cm_s"]
    )
    ...
    energy_supercomoving_cgs_erg = (
        np.asarray(fluid.Energy_code[interior], dtype=float) * unit_conversion["energy_cgs_erg"]
    )
    ...
    state = {
        ...
        "xHI": as_named_array(
            fluid.xHI[interior] if hasattr(fluid, "xHI") else np.ones(int(par.mesh.grid_cells)),
        ),
    ...
    if state["thermal_coupling"]:
        ...
        specific_kinetic_supercomoving = 0.5 * velocity_supercomoving_cgs_cm_s**2
        ...
        specific_internal_physical = (
            np.maximum(
                ...
                - rotational_specific_code * unit_conversion["velocity_cgs_cm_s"] ** 2,
                0.0,
            )
            / scaling["temperature_factor"]
        )
        ...
        state["specific_kinetic_energy_supercomoving_cgs_erg_g"] = specific_kinetic_supercomoving
        state["specific_kinetic_energy_cgs_erg_g"] = 0.5 * state["vel_cgs_cm_s"] ** 2
        state["specific_total_energy_cgs_erg_g"] = (
            specific_internal_physical + state["specific_kinetic_energy_cgs_erg_g"]
        )
...
def _fast_update_temperature_from_energy(state: Any) -> Any:
    ...
    if temperature_floor > 0.0:
        energy_floor = (
            BOLTZMANN_CONSTANT_CGS
            * temperature_floor
            / ((state["gamma"] - 1.0) * np.maximum(state["mu"], 1.0e-99) * PROTON_MASS_CGS)
        )
    ...
    if "specific_energy_cgs_erg_g" in state:
        ...
        state["specific_total_energy_cgs_erg_g"] = (
            state["specific_energy_cgs_erg_g"] + state["specific_kinetic_energy_cgs_erg_g"]
        )
    state["temperature_cgs_K"] = (
        (state["gamma"] - 1.0)
        * state["mu"]
...
def _fast_apply_thermal_source(state: Any, thermal_rate_cgs_erg_cm3_s: Any, dt_s: Any) -> Any:
    ...
    energy_update[active] = np.asarray(thermal_rate_cgs_erg_cm3_s)[active] / rho[active] * dt_s
...
def _apply_compton_only_source(state: Any, dt_s: Any) -> Any:
    ...
    specific_heat = BOLTZMANN_CONSTANT_CGS / (
        (state["gamma"] - 1.0) * state["mu"] * PROTON_MASS_CGS
    )
    cmb_temperature = state["cmb_temperature_0_cgs_K"] * (
    ...
    state["specific_total_energy_cgs_erg_g"] = (
        specific_heat * temperature + state["specific_kinetic_energy_cgs_erg_g"]
    )
    state["specific_energy_cgs_erg_g"] = specific_heat * temperature
    _fast_update_temperature_from_energy(state)


def _record_coupled_implicit_failure(
    reason: Any,
    determinant: Any,
    residual_energy: Any,
    residual_x: Any,
    ...
    residual_norm = np.maximum(
        np.abs(residual_energy),
        np.abs(residual_x),
    )
    unconverged = active_cells & (
        ~np.isfinite(residual_energy) | ~np.isfinite(residual_x) | (residual_norm > tolerance)
    )
    unconverged_cells = [
        {
            "cell": int(cell),
    ...
    _, _, _, _, failure_trial = residual(log_energy, logit_x)
    heating_rate = thermal_rate(
        dict(failure_trial, atomic_cooling=False),
        ngamma_cgs_cm3,
    )
    ...
    state["_implicit_failure"] = {
        ...
        "jacobian_determinant": (None if determinant is None else float(determinant[index])),
...
def _coupled_newton_line_search(
    delta_energy: Any,
    delta_x: Any,
    finite_delta: Any,
    current_norm: Any,
    floor_constrained: Any,
    log_energy: Any,
    logit_x: Any,
    residual: Any,
    residual_energy: Any,
    ...
    accepted = np.zeros_like(finite_delta, dtype=bool)
    for damping in (1.0, 0.5, 0.25, 0.125, 0.0625, 0.03125, 0.015625, 0.0078125):
        ...
        trial_energy_residual, trial_x_residual = residual(
            trial_log_energy,
            trial_logit_x,
        )[:2]
        trial_norm = np.where(
        ...
        improve = (
            ...
            & (trial_norm <= current_norm * (1.0 - (1.0e-4 if trust_region else 0.0)))
        )
        if np.any(improve):
...
def _run_coupled_newton_iterations(
    max_iterations: Any,
    log_energy: Any,
    logit_x: Any,
    residual_energy: Any,
    ...
    active_cells: Any,
    residual: Any,
    floor_constraint: Any,
    record_failure: Any,
    energy_residual_acceptance: Any,
    xhi_residual_tolerance: Any,
    physical_energy_floor: Any,
    finite_difference_step: Any,
    trust_region: Any,
    trust_radius: Any,
    trust_radius_min: Any,
) -> Any:
    for _ in range(int(max_iterations)):
        _, _, _, _, current_trial = residual(log_energy, logit_x)
        floor_constrained = floor_constraint(current_trial)
        ...
        floor_log_energy[positive_floor] = (
            np.log(physical_energy_floor[positive_floor]) + finite_difference_step
        )
        ...
        energy_plus, x_plus = residual(
            forward_log_energy,
            logit_x,
        )[:2]
        energy_x_plus, x_x_plus = residual(
            log_energy,
        ...
        delta_energy[good] = (
            -residual_energy[good] * jacobian_22[good] + jacobian_12[good] * residual_x[good]
        ) / determinant[good]
        delta_x[good] = (
            jacobian_21[good] * residual_energy[good] - jacobian_11[good] * residual_x[good]
        ) / determinant[good]
        delta_x[scalar_chemistry] = -residual_x[scalar_chemistry] / jacobian_22[scalar_chemistry]
        delta_energy[scalar_energy] = -residual_energy[scalar_energy] / jacobian_11[scalar_energy]
        finite_delta = solvable & np.isfinite(delta_energy) & np.isfinite(delta_x)
        if trust_region:
        ...
        accepted = _coupled_newton_line_search(
            delta_energy,
            delta_x,
            finite_delta,
            current_norm,
        ...
        converged |= accepted & (
            (floor_constrained & (np.abs(residual_x) <= xhi_residual_tolerance))
            | (
                ~floor_constrained
        ...
        if trust_region:
            trust_radius[accepted] = np.minimum(
                4.0,
                trust_radius[accepted] * 1.5,
            )

    return True


def _coupled_source_residual(
    log_energy: Any,
    logit_x: Any,
    state: Any,
    kinetic: Any,
    ...
    dt_value: Any,
    rho_for_update: Any,
    energy_residual_scale: Any,
) -> Any:
    energy = np.exp(np.clip(log_energy, np.log(energy_floor), 700.0))
    xhi = 1.0 / (1.0 + np.exp(-np.clip(logit_x, -700.0, 700.0)))
    trial = dict(state)
...
def _coupled_source_floor_constraint(
    trial: Any,
    ...
    return (
        ...
        & (
            ...
            <= temperature_floor * (1.0 + state.get("temperature_floor_tolerance", 1.0e-6))
        )
        & (thermal <= 0.0)
    )


def _prepare_coupled_source_inputs(state: Any, dt_s: Any) -> Any:
...
def _coupled_implicit_source_update(
    ...
    max_iterations: Any = 32,
    *,
    trust_region: Any = False,
    absolute_temperature_tolerance: Any = 0.0,
    absolute_xhi_tolerance: Any = 0.0,
) -> Any:
    ...
    absolute_energy_tolerance = (
        BOLTZMANN_CONSTANT_CGS
        * max(float(absolute_temperature_tolerance), 0.0)
        / (
            (state["gamma"] - 1.0)
    ...
    small_energy = (absolute_energy_tolerance > 0.0) & (
    ...
    if dt_value == 0.0:
        ...
        state["specific_total_energy_cgs_erg_g"] = state["specific_energy_cgs_erg_g"] + kinetic
        state["xHI"] = np.where(active_cells, x_old, state["xHI"])
        _fast_update_temperature_from_energy(state)
        return True

    residual = partial(
        _coupled_source_residual,
        state=state,
        kinetic=kinetic,
        ...
        x_old=x_old,
        dt_value=dt_value,
        rho_for_update=rho_for_update,
        energy_residual_scale=energy_residual_scale,
    )
    log_energy = np.log(energy_old)
    clipped_x_old = np.clip(x_old, x_floor, 1.0 - x_floor)
    logit_x = np.log(clipped_x_old / (1.0 - clipped_x_old))
    residual_energy, residual_x, _, _, initial_trial = residual(log_energy, logit_x)
    floor_constraint = partial(
        _coupled_source_floor_constraint,
        temperature_floor=temperature_floor,
        active_cells=active_cells,
        state=state,
        ngamma_cgs_cm3=ngamma_cgs_cm3,
    )
    floor_constrained = floor_constraint(initial_trial)
    ...
    converged[finite] = (np.abs(residual_energy[finite]) <= energy_residual_acceptance[finite]) & (
    ...
    record_failure = partial(
        _record_coupled_implicit_failure,
        state=state,
        active_cells=active_cells,
        tolerance=tolerance,
        residual=residual,
    ...
    if not _run_coupled_newton_iterations(
        max_iterations,
        log_energy,
        logit_x,
        residual_energy,
    ...
    final_residual_energy, final_residual_x, _, _, final_trial = residual(
        log_energy,
        logit_x,
    )
    floor_constrained = floor_constraint(final_trial)
    ...
    _, _, energy, xhi, trial = residual(log_energy, logit_x)
    state["specific_energy_cgs_erg_g"] = np.where(
        active_cells,
        energy,
        state["specific_energy_cgs_erg_g"],
...
def _copy_fast_source_state(state: Any) -> Any:
    """Copy a numeric source state for a trial implicit update."""
    return copy.deepcopy(state)


def _source_array_scale(state: Any, name: str, active_indices: Any) -> Any:
...
def _explicit_source_state_update(state: Any, remaining_s: Any, par: Any) -> Any:
    ...
    while remaining_s > 0.0:
        ...
        check_source_temperature(
            ...
            par,
            temperature_before,
            stage="hydrogen explicit source",
...
def _split_implicit_source_state_update(state: Any, dt_s: Any, par: Any) -> Any:
    ...
    while remaining_s > 0.0:
        ...
        before, trial, candidate_dt_s = _split_implicit_trial(
            state,
            candidate_dt_s,
            active,
        )
...
def _split_implicit_trial(state: Any, candidate_dt_s: Any, active: Any) -> Any:
    while True:
        ...
        if trial["thermal_coupling"]:
            _fast_apply_thermal_source(
                trial,
                thermal_rate(trial, trial.get("ngamma_cgs_cm3")),
                candidate_dt_s,
            )
        if trial["recombination"] or trial["collisional_ionization"]:
            ionization_fraction_implicit_update(
                trial,
                trial.get("ngamma_cgs_cm3"),
                candidate_dt_s,
            )
        ...
        limiter_baseline_energy = _split_energy_baseline(trial, old_energy)
        with np.errstate(divide="ignore", invalid="ignore"):
            relative_energy_change = np.abs(new_energy - limiter_baseline_energy) / np.maximum(
                np.abs(limiter_baseline_energy),
                1.0e-30,
            )
        ...
        if max_energy_change <= 0.1:  # noqa: PLR2004
            return before, trial, candidate_dt_s
        candidate_dt_s *= 0.5
...
def _split_energy_baseline(trial: Any, old_energy: Any) -> Any:
    temperature_floor = float(trial.get("temperature_floor_cgs_K", 0.0) or 0.0)
    if not trial["thermal_coupling"] or temperature_floor <= 0.0:
        return old_energy.copy()
    floor_energy = (
        BOLTZMANN_CONSTANT_CGS
        * temperature_floor
        / ((trial["gamma"] - 1.0) * np.maximum(trial["mu"], 1.0e-99) * PROTON_MASS_CGS)
    )
    return np.maximum(old_energy, floor_energy)


def _source_relative_change(before: Any, after: Any) -> Any:
...
def _implicit_state_difference(coarse: Any, fine: Any) -> Any:
...
def _set_fast_source_state(target: Any, source: Any) -> None:
...
def _adaptive_trial(
    ...
    max_refinements: Any,
    trust_region: Any,
    absolute_temperature_tolerance: Any,
    absolute_xhi_tolerance: Any,
) -> Any:
    last_failure = None
    for _refinement in range(int(max_refinements) + 1):
        ...
        difference = _implicit_state_difference(coarse, fine) if coarse_ok and fine_ok else np.inf
        if coarse_ok and fine_ok and difference <= convergence_tolerance:
            _set_fast_source_state(state, fine)
            return True, candidate_dt_s, 2, last_failure
        candidate_dt_s *= 0.5
    return False, candidate_dt_s, 0, last_failure
...
def _adaptive_coupled_implicit_source_update_group(
    ...
    ngamma_cgs_cm3: Any = None,
    tolerance: Any = 1e-06,
    convergence_tolerance: Any = None,
    max_iterations: Any = 32,
    max_refinements: Any = 4,
    *,
    trust_region: Any = False,
    absolute_temperature_tolerance: Any = 0.0,
    absolute_xhi_tolerance: Any = 0.0,
    ...
    while remaining_s > zero_time_s:
        candidate_dt_s = min(trial_dt_s, remaining_s)
        accepted, accepted_dt_s, steps, last_failure = _adaptive_trial(
            state,
            candidate_dt_s,
            ngamma_cgs_cm3,
            tolerance,
            ...
            max_refinements,
            trust_region,
            absolute_temperature_tolerance,
            absolute_xhi_tolerance,
        )
        ...
        remaining_s -= accepted_dt_s
        total_source_steps += steps
        trial_dt_s = min(2.0 * accepted_dt_s, remaining_s)
    return True, total_source_steps


def _source_stiffness_groups(state: Any, dt_s: Any, ngamma_cgs_cm3: Any = None) -> Any:
...
def _slice_source_cells(state: Any, indices: Any) -> Any:
...
def _slice_source_photons(ngamma_cgs_cm3: Any, indices: Any, count: Any) -> Any:
    if ngamma_cgs_cm3 is None:
        return None
...
def _merge_source_cells(state: Any, subset: Any, indices: Any) -> Any:
    for key in (
        "specific_energy_cgs_erg_g",
...
def _fast_sync_state_to_fluid(state: Any, fluid: Any, par: Any) -> Any:
    ...
    if state.get("thermal_coupling", False):
        _fast_sync_thermal_state(state, fluid, par, code, interior, active)
    if (
        state.get("hydrogen_update_mu", False)
        and hasattr(fluid, "xHI")
        and getattr(getattr(fluid, "eos", None), "gamma", None) is not None
    ):
        fluid.SetHydrogenMu(
            hydrogen_mass_fraction=state["hydrogen_mass_fraction"],
        )
...
def _fast_sync_thermal_state(
    ...
    specific_energy_code_factor = float(code.unit_conversion["velocity_cgs_cm_s"]) ** 2
    ...
    specific_total_energy = (
        specific_internal_energy_code + specific_kinetic_energy_code + rotational_specific_code
    )
    rho_runtime_code, _, pre_runtime_code, _, _ = _canonical_fluid_primitive_arrays(fluid, par)
    pressure = (
...
def c2ray_source_state(mesh: Any, fluid: Any, par: Any) -> Any:
    """Return the numeric source state used by the C²-Ray integrator."""
    return _fast_source_state(mesh, fluid, par)


def sync_c2ray_state(state: Any, fluid: Any, par: Any) -> Any:
...
def _try_compton_only_source(
    ...
    enabled = all(
        (
            ...
            state["compton_cmb_enabled"],
            not state["recombination"],
            not state["collisional_ionization"],
            not state.get("atomic_cooling", True),
            not getattr(par, "radiative_transfer", False),
            not getattr(par, "hydrogen_radiation_field", False),
            state.get("ngamma_cgs_cm3") is None,
        ),
    )
    if not enabled:
        return None
...
def _finish_fast_source_update(
    ...
    energy: Any = getattr(par, "ionizing_photon_energy_cgs_erg", None)
    if energy is None:
...
def _fast_prepare_substep(state: Any) -> Any:
    ...
    if state["thermal_coupling"]:
        _fast_update_temperature_from_energy(state)
    return np.asarray(state["temperature_cgs_K"], dtype=float).copy()


def _fast_choose_substep(state: Any, par: Any, remaining_s: Any, zero_time_s: Any) -> Any:
    """Choose and validate one adaptive source timestep."""
    sub_dt_s, thermal_rate = get_timestep(
        state,
        state.get("ngamma_cgs_cm3"),
        remaining_s,
    ...
    if not np.isfinite(sub_dt_s) or sub_dt_s <= zero_time_s:
        _raise_invalid_source_timestep(state, sub_dt_s, remaining_s, thermal_rate)
    return min(sub_dt_s, remaining_s), thermal_rate


def _fast_apply_chemistry_step(state: Any, par: Any, thermal_rate: Any, sub_dt_s: Any) -> Any:
    """Apply thermal and ionization source updates for one substep."""
    if state["thermal_coupling"]:
        _fast_apply_thermal_source(state, thermal_rate, sub_dt_s)
    if (
        state["recombination"]
...
class HydrogenNetwork(ThermochemistryNetwork):
    ...
    def enabled(self: Any, fluid: Any, par: Any) -> Any:
        return thermochemistry_enabled(fluid, par)

    def radiation_enabled(self: Any, fluid: Any, par: Any) -> Any:
        return thermochemistry_radiation_enabled(fluid, par)

    def radiation_evolution_enabled(self: Any, fluid: Any, par: Any) -> Any:
    ...
    def source_state(self: Any, mesh: Any, fluid: Any, par: Any) -> Any:
        return source_state(mesh, fluid, par)

    def ionization_fraction_rate(self: Any, state: Any, ngamma_cgs_cm3: Any) -> Any:
        return ionization_fraction_rate(state, ngamma_cgs_cm3)

    def thermal_rate(self: Any, state: Any, ngamma_cgs_cm3: Any) -> Any:
    ...
    def update_temperature_from_energy(self: Any, state: Any) -> Any:
    ...
    def apply_state(self: Any, state: Any, fluid: Any, par: Any) -> Any:
        return apply_state(state, fluid, par)

    def get_source_timestep_fast(self: Any, mesh: Any, fluid: Any, par: Any, remaining: Any) -> Any:
