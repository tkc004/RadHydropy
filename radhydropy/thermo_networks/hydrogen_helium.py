...
def _rates(state: Any, ngamma_cgs_cm3: Any) -> Any:
    ...
    cooling += ne * _gamma_bremsstrahlung(T) * (nH * xHII + nHe * (xHeII + 4.0 * xHeIII))
    ...
    if metal_table is not None:
        ...
        if getattr(metal_table, "is_hm12_uv_background", False) and max_heating_density is not None:
...
def source_state(mesh: Any, fluid: Any, par: Any) -> Any:
    ...
    eps = {
        "HI": quantity_or_code_to_cgs(
            par.radiation_group_epsilon_gamma,
            code,
            "erg",
            "energy_cgs_erg",
        ),
    ...
    specific_internal = (
        np.maximum(
            total_super - 0.5 * velocity_super**2 - rotational_super,
            0.0,
        )
    ...
    state = {
        ...
        "radius_kpc": to_unit_value(x_runtime_code[interior], code.length_unit)
        * scaling["scale_factor"]
        / 3.08567758e21,
        "rho_cgs_g_cm3": source.rho_cgs_g_cm3,
        "active": thermochemistry_active_mask(source.rho_cgs_g_cm3, par, scaling["density_factor"]),
        "temperature_cgs_K": source.temperature_cgs_K,
        "specific_energy_cgs_erg_g": source.specific_energy_cgs_erg_g,
        "rotational_specific_energy_code": rotational_super
        / code.unit_conversion["velocity_cgs_cm_s"] ** 2,
        "gamma": gamma,
        "hydrogen_mass_fraction": getattr(par, "hydrogen_mass_fraction", 0.7),
        "helium_mass_fraction": getattr(par, "helium_mass_fraction", 0.28),
        "xHI": source.xHI_dimensionless,
        "xHeI": xHeI,
        "xHeIII": xHeIII,
        "sigma_gamma_cgs_cm2": sigma,
        "epsilon_gamma_cgs_erg": eps,
        ...
        "absolute_tolerance": getattr(par, "absolute_tolerance", 1.0e-10),
...
def apply_state(state: Any, fluid: Any, par: Any) -> None:
    ...
    if state.get("time_s") is not None:
        time_runtime_code = from_unit_value(state["time_s"], code.time_unit)
        if getattr(par, "supercomoving_coordinates", False):
            fluid.tau_supercomoving_code = time_runtime_code
        else:
            fluid.time_proper_code = time_runtime_code
    fluid.xHI[i] = state["xHI"]
    fluid.xHeI[i] = state["xHeI"]
    fluid.xHeII[i] = state["xHeII"]
    fluid.xHeIII[i] = state["xHeIII"]
    ...
    if hasattr(fluid, "Mass_code") and hasattr(fluid, "Energy_code"):
        ...
        specific_code += state.get(
            "rotational_specific_energy_code",
            np.zeros_like(specific_code),
        )
...
class HydrogenHeliumNetwork(ThermochemistryNetwork):
    ...
    def get_source_timestep_fast(
        ...
        physical_remaining_s = (
            to_unit_value(remaining, code.time_unit) * state["source_scale_factor"] ** 2
        )
