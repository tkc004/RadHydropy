...
def proper_code_state_from_physical(
    *,
    ...
    values = {
        "rho_proper_code": _physical_value("rho_unyt", rho_unyt, code_units.density_unit),
        "vel_proper_code": _physical_value("vel_unyt", vel_unyt, code_units.velocity_unit),
        ...
        "time_proper_code": _physical_value("time_unyt", time_unyt, code_units.time_unit),
    }
    return ProperCodeState(**values)
...
def cgs_source_state_from_code(
    *,
    ...
    if isinstance(fluid, ProperCodeState):
        ...
        specific_energy = fluid.specific_energy_proper_code
        time = fluid.time_proper_code
    elif isinstance(fluid, SupercomovingCodeState):
        ...
        specific_energy = fluid.specific_energy_supercomoving_code
        time = fluid.tau_supercomoving_code
    else:
        raise UnitBoundaryError("fluid must be a representation-specific typed code state")
    ...
    if fluid.Energy_code is not None and fluid.Mass_code is not None:
        ...
        converted["specific_energy_cgs_erg_g"] = (
            specific_energy * scales["specific_energy_cgs_erg_g"]
        )
    else:
        raise UnitBoundaryError(
