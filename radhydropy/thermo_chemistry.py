"""Thermo-chemistry source updates.

This module owns operator-split chemistry and thermal source updates used by
the finite-volume solver.  The current rate network is hydrogen-only, but the
public function names are intentionally element-neutral so broader chemistry
networks can be added behind the same interface.
"""
