...
class Gravity:
    ...
    def _tabulated_quantity(self, values, coordinate, label):
        ...
        value_unit = (
            _potential_unit(code_units) if label == "potential" else _acceleration_unit(code_units)
        )
    ...
    def self_acceleration_on_mesh(self, mesh, rho, par):
        ...
        if mesh.coordsys == "cartesian":
            ...
            for index in range(first + 1, last):
                result[index] = (
                    result[index - 1] - 4.0 * np.pi * g_code * rho[index - 1] * dx[index - 1]
                )
            if first < last:
    ...
    def advance_dark_matter(
        ...
        return self.dark_matter.step(
            ...
            crossing_batch_fraction=getattr(
                par,
                "dark_matter_crossing_batch_fraction",
                0.0,
            ),
